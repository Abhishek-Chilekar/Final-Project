...
route.patch('/:id',async(req,res)=>{
    ...
    try{
        if(data && data.chats && data.senderId && data.receiverId){
            data.chats.map((r)=>{
                if(r.owner != null && r.message.messageId != null && r.message.messageContent != null && r.message.type !=null && r.message.timeline !=null && r.message.imagePath !=null){
                    r.owner = r.owner;
                    ...
                    r.message.imagePath = textencrypt(r.message.imagePath);
            ...
            if(data && data.chats && data.senderId && data.receiverId){
                data.chats.map((r)=>{
                    if(r.owner != null && r.message.messageId != null && r.message.messageContent != null && r.message.type !=null && r.message.timeline !=null && r.message.imagePath !=null){
                        ...
                        r.message.imagePath = textdecrypt(r.message.imagePath);
...
route.get("/",async (req,res)=>{
    try
    {
        ...
        const data = snapshot.docs.map((doc)=>{
            const data = doc.data();
            data.chats.map((r)=>{
                if(r.owner && r.message.messageId && r.message.messageContent && r.message.type && r.message.timeline && r.message.imagePath){
                    r.owner = r.owner;
                    r.message.messageId = textdecrypt(r.message.messageId);
                    r.message.messageContent = textdecrypt(r.message.messageContent);
                    r.message.type = textdecrypt(r.message.type);
...
//Delete Group
route.delete("/:chatId", async (req,res)=>{
    try{
        await privateChat.doc(req.params.chatId).delete()
        .catch((err)=>{
            res.send({
                msg:err.message
            });
        });
        res.send({msg:"Chat Window deleted"});
    }
    catch(err){
