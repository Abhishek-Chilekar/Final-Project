...
const AddEvent = ({popupstate,reload}) => {
    ...
    const upload = async (props) => {
        setDisabled(true);
        const newDate = new Date(props.from);
        newDate.setHours(0, 0, 0, 0);
        const endDate = new Date(props.till);
        endDate.setHours(0, 0, 0, 0);
        if(newDate.toISOString() > endDate.toISOString()){
            setError("End Date cannot be before Start Date")
        }
    ...
    return (<div className={style.overlay}>
        <div className={style.popup}>
            <div className={style.header}>
                ...
                <input accept = "image/png, image/jpeg, image/jpg" type="file" name="file" id="file" className={style.file} onChange={(e) => { setFile(e.target.files[0]) }} />
                <h1>Events</h1>
            </div>
