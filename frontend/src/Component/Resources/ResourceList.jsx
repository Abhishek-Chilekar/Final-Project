...
const ResourceList = ({select,reload,setReload}) =>{
    ...
    return(<div className={style.resourceList}>{list.map((c)=>{
        return (select == "All" || select == c.type) && (user.role != "Student"||(c.branch.toLowerCase() == "all" ||c.branch.toLowerCase() == user.branch.toLowerCase())) && <Resource content={c} reload={()=>setReload()}/>
    })}</div>)
}
